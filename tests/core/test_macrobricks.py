...
from finbricklab import Scenario, ABrick, LBrick, MacroBrick, Registry
...
class TestMacroBrick:
    ...
    def test_macrobrick_expand_simple(self):
        ...
        # Create test bricks
        brick1 = ABrick(id="brick1", name="Brick 1", kind=K.A_CASH, spec={"initial_balance": 1000.0})
        brick2 = ABrick(id="brick2", name="Brick 2", kind=K.A_CASH, spec={"initial_balance": 2000.0})
        
        # Create registry
        registry = Registry(
            bricks={"brick1": brick1, "brick2": brick2},
            macrobricks={}
        )
    ...
    def test_macrobrick_expand_nested(self):
        ...
        brick1 = ABrick(id="brick1", name="Brick 1", kind=K.A_CASH, spec={"initial_balance": 1000.0})
        brick2 = ABrick(id="brick2", name="Brick 2", kind=K.A_CASH, spec={"initial_balance": 2000.0})
        brick3 = ABrick(id="brick3", name="Brick 3", kind=K.A_CASH, spec={"initial_balance": 3000.0})
    ...
    def test_macrobrick_cycle_detection(self):
        ...
        with pytest.raises(ConfigError, match="Cycle detected in MacroBrick membership"):
            Registry(
                bricks={"brick1": brick1},
                macrobricks={"A": mb_a, "B": mb_b}
            )
...
class TestRegistry:
    ...
    def test_registry_id_conflicts(self):
        ...
        brick1 = ABrick(id="conflict", name="Brick", kind=K.A_CASH, spec={"initial_balance": 1000.0})
        mb1 = MacroBrick(id="conflict", name="MacroBrick", members=[])
        
        with pytest.raises(ConfigError, match="ID conflicts between bricks and MacroBricks"):
            Registry(
                bricks={"conflict": brick1},
                macrobricks={"conflict": mb1}
            )
    ...
    def test_registry_reserved_prefix_validation(self):
        ...
        brick1 = ABrick(id="b:reserved", name="Brick 1", kind=K.A_CASH, spec={"initial_balance": 1000.0})
        mb1 = MacroBrick(id="mb:reserved", name="MacroBrick 1", members=["b:reserved"])
        
        # Create registry (should succeed)
        registry = Registry(
            bricks={"b:reserved": brick1},
            macrobricks={"mb:reserved": mb1}
        )
...
class TestScenarioIntegration:
    ...
    def test_scenario_execution_set_resolution(self):
        ...
        cash = ABrick(id="cash", name="Cash", kind=K.A_CASH, spec={"initial_balance": 10000.0})
        ...
        primary_mb = MacroBrick(id="primary", name="Primary Residence", members=["house", "mortgage"])
    ...
    def test_scenario_overlap_handling(self):
        ...
        primary_mb = MacroBrick(id="primary", name="Primary", members=["house", "mortgage"])
        property_mb = MacroBrick(id="property", name="Property", members=["house", "cash"])
    ...
    def test_scenario_struct_aggregation(self):
        ...
        assert np.array_equal(struct_output["cash_in"], house_output["cash_in"] + mortgage_output["cash_in"])
        assert np.array_equal(struct_output["cash_out"], house_output["cash_out"] + mortgage_output["cash_out"])
        assert np.array_equal(struct_output["asset_value"], house_output["asset_value"] + mortgage_output["asset_value"])
        assert np.array_equal(struct_output["debt_balance"], house_output["debt_balance"] + mortgage_output["debt_balance"])
    ...
    def test_scenario_cash_account_requirement(self):
        ...
        with pytest.raises(AssertionError, match="Scenario expects exactly one cash account brick"):
...
class TestAggregationCorrectness:
    ...
    def test_aggregation_manual_vs_automatic(self):
        ...
        # Run scenario
        results = scenario.run(start=date(2026, 1, 1), months=12, selection=["primary", "cash"])
        ...
        manual_asset_value = house_output["asset_value"] + mortgage_output["asset_value"]
        manual_debt_balance = house_output["debt_balance"] + mortgage_output["debt_balance"]
    ...
    def test_portfolio_totals_deduplication(self):
        ...
        results = scenario.run(start=date(2026, 1, 1), months=12, selection=["primary", "property"])
...
class TestAggregationInvariants:
    ...
    def test_union_less_than_or_equal_to_sum_with_overlap(self):
        ...
        portfolio_assets = portfolio_totals.assets.iloc[-1] if hasattr(portfolio_totals.assets, 'iloc') else portfolio_totals.assets[-1]
        ...
        assert portfolio_assets <= sum_assets + 1e-8, f"Portfolio assets {portfolio_assets} should be ≤ sum {sum_assets}"
    ...
    def test_disjoint_equality(self):
        ...
        cash = ABrick(id="cash", name="Cash", kind=K.A_CASH, spec={"initial_balance": 10000.0})
        house1 = ABrick(id="house1", name="House 1", kind=K.A_PROPERTY_DISCRETE,
                       spec={"initial_value": 400000.0, "fees_pct": 0.05, "appreciation_pa": 0.03})
        house2 = ABrick(id="house2", name="House 2", kind=K.A_PROPERTY_DISCRETE,
                       spec={"initial_value": 300000.0, "fees_pct": 0.05, "appreciation_pa": 0.025})
        ...
        results = scenario.run(start=date(2026, 1, 1), months=12, selection=["primary", "secondary", "cash"])
        ...
        assert abs(portfolio_assets - sum_assets) < 1e-8, f"Portfolio assets {portfolio_assets} should equal sum {sum_assets}"
    ...
    def test_execution_order_deterministic(self):
        ...
        assert order1 == order2, f"Execution order should be deterministic: {order1} vs {order2}"
