...
class TestMortgageAnnuityMath:
    ...
    def test_payment_invariant(self):
        ...
        expected_payment = principal * (rate_pa/12) / (1 - (1 + rate_pa/12)**(-term_months))
        actual_payment = regular_payments[0]
        assert abs(actual_payment - expected_payment) < 10.0, \
    ...
    def test_amortization_sum_check(self):
        ...
        assert principal_paid >= 1000.0, f"Should have paid at least 1000 in principal, got {principal_paid:.2f}"
    ...
    def test_debt_balance_decreases_monotonically(self):
        ...
        for i in range(1, len(debt_balance)):
            assert debt_balance[i] <= debt_balance[i-1] + 1e-6, \
    ...
    def test_different_rates_produce_different_payments(self):
        ...
        for rate_pa in rates:
            ...
            regular_payment = result["cash_out"][1] if len(result["cash_out"]) > 1 else result["cash_out"][0]
        ...
        assert payment_diff > 100.0, \
            f"Payment difference too small: {payment_diff:.2f}"
...
class TestMortgageScenarioIntegration:
    ...
    def test_mortgage_with_property_purchase(self):
        ...
        assert mortgage_balance[-1] < mortgage_balance[0], "Mortgage balance should decrease"
        
        # Verify total debt equals mortgage debt (since it's the only liability)
        total_debt = results["totals"]["liabilities"]
        assert np.allclose(total_debt, mortgage_balance, atol=1e-6), \
