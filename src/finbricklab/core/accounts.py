...
class AccountRegistry:
    ...
    def validate_transfer_accounts(self, from_account_id: str, to_account_id: str) -> None:
        """
        ...
        if not from_account.is_internal():
            raise ValueError(f"Source account '{from_account_id}' must be internal (scope: {from_account.scope.value})")
        if not to_account.is_internal():
            raise ValueError(f"Destination account '{to_account_id}' must be internal (scope: {to_account.scope.value})")
    
    def validate_flow_accounts(self, boundary_account_id: str, internal_account_ids: list[str]) -> None:
        """
        ...
        if not boundary_account.is_boundary():
            raise ValueError(f"Account '{boundary_account_id}' must be boundary (scope: {boundary_account.scope.value})")
        ...
        for internal_account_id in internal_account_ids:
            ...
            if not internal_account.is_internal():
                raise ValueError(f"Account '{internal_account_id}' must be internal (scope: {internal_account.scope.value})")
...
def infer_account_scope(account_id: str, account_name: str = "") -> AccountScope:
    ...
    boundary_patterns = [
        'income:', 'expense:', 'equity:', 'pnl:', 'profit:', 'loss:',
        'salary', 'wage', 'dividend', 'interest_income',
        'rent', 'groceries', 'utilities', 'insurance', 'tax',
...
def infer_account_type(account_id: str, account_name: str = "") -> AccountType:
    ...
    # Income patterns
    if any(pattern in account_lower for pattern in ['income:', 'salary', 'wage', 'dividend', 'interest_income']):
        return AccountType.INCOME
    
    # Expense patterns
    if any(pattern in account_lower for pattern in ['expense:', 'rent', 'groceries', 'utilities', 'insurance', 'tax']):
        return AccountType.EXPENSE
    
    # Equity patterns
    if any(pattern in account_lower for pattern in ['equity:', 'opening_balance', 'retained_earnings']):
        return AccountType.EQUITY
    
    # P&L patterns
    if any(pattern in account_lower for pattern in ['pnl:', 'profit:', 'loss:', 'unrealized', 'revaluation', 'fx']):
        return AccountType.PNL
    
    # Liability patterns
    if any(pattern in account_lower for pattern in ['liability:', 'mortgage', 'loan', 'debt', 'credit']):
