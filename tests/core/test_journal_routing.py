...
class TestJournalRouting:
    ...
    def test_split_income_two_cash_accounts(self):
        ...
        checking = entity.new_ABrick('checking', 'Checking', K.A_CASH, {'initial_balance': 0.0, 'interest_pa': 0.0})
        savings = entity.new_ABrick('savings', 'Savings', K.A_CASH, {'initial_balance': 0.0, 'interest_pa': 0.0})
        
        # Create income that splits 70% to checking, 30% to savings
        income = entity.new_FBrick('income', 'Salary', K.F_INCOME_FIXED, {
            'amount_monthly': 3000.0
        })
        
        # Create expense paid from checking only
        expense = entity.new_FBrick('expense', 'Living', K.F_EXPENSE_FIXED, {
            'amount_monthly': 1000.0
        })
        
        scenario = entity.create_scenario('multi-cash', 'Multi-cash routing', ['checking', 'savings', 'income', 'expense'])
    ...
    def test_default_routing_when_no_links(self):
        ...
        income = entity.new_FBrick('income', 'Salary', K.F_INCOME_FIXED, {'amount_monthly': 500.0})
        expense = entity.new_FBrick('expense', 'Rent', K.F_EXPENSE_FIXED, {'amount_monthly': 200.0})
        
        scenario = entity.create_scenario('defaults', 'Defaults', ['main', 'side', 'income', 'expense'])
    ...
    def test_journal_compilation_with_multiple_accounts(self):
        ...
        registry.register_account(Account("checking", "Checking", AccountScope.INTERNAL, AccountType.ASSET))
        registry.register_account(Account("savings", "Savings", AccountScope.INTERNAL, AccountType.ASSET))
        registry.register_account(Account("Income:Salary", "Salary", AccountScope.BOUNDARY, AccountType.INCOME))
        ...
        # Check postings
        income_posting = next(p for p in entry.postings if p.account_id == "Income:Salary")
    ...
    def test_journal_validation_with_multiple_accounts(self):
        ...
        entry = JournalEntry(
            ...
            postings=[
                Posting("Income:Salary", create_amount(-1000, "EUR"), {"type": "income"}),
    ...
    def test_currency_precision_with_routing(self):
        ...
        entry = JournalEntry(
            ...
            timestamp=date(2026, 1, 1),
            postings=[
                Posting("Income:Salary", create_amount(-1234.56, "EUR"), {"type": "income"}),
                Posting("checking", create_amount(864.19, "EUR"), {"type": "checking_in"}),  # 70%
                Posting("savings", create_amount(370.37, "EUR"), {"type": "savings_in"})   # 30%
            ],
            metadata={"type": "income_split"}
        )
    ...
    def test_scenario_with_multiple_cash_accounts(self):
        ...
        checking = entity.new_ABrick('checking', 'Checking', K.A_CASH, {'initial_balance': 1000.0, 'interest_pa': 0.02})
        savings = entity.new_ABrick('savings', 'Savings', K.A_CASH, {'initial_balance': 5000.0, 'interest_pa': 0.03})
        investment = entity.new_ABrick('investment', 'Investment', K.A_CASH, {'initial_balance': 0.0, 'interest_pa': 0.01})
        
        # Create various flows
        salary = entity.new_FBrick('salary', 'Salary', K.F_INCOME_FIXED, {'amount_monthly': 5000.0})
        bonus = entity.new_FBrick('bonus', 'Bonus', K.F_INCOME_FIXED, {'amount_monthly': 1000.0})
        rent = entity.new_FBrick('rent', 'Rent', K.F_EXPENSE_FIXED, {'amount_monthly': 2000.0})
        groceries = entity.new_FBrick('groceries', 'Groceries', K.F_EXPENSE_FIXED, {'amount_monthly': 800.0})
        
        scenario = entity.create_scenario('multi-account', 'Multi-account scenario', 
                                        ['checking', 'savings', 'investment', 'salary', 'bonus', 'rent', 'groceries'])
    ...
    def test_journal_entries_with_different_currencies(self):
        ...
        usd_entry = JournalEntry(
            ...
            postings=[
                Posting("Income:Salary", create_amount(-1200, "USD"), {"type": "income"}),
