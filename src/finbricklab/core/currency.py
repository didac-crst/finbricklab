...
class Currency:
    ...
    def __init__(self, code: str, decimals: int = 2, rounding: RoundingPolicy = RoundingPolicy.BANKERS):
...
class Amount:
    ...
    def __add__(self, other: Amount) -> Amount:
        if self.currency.code != other.currency.code:
            raise ValueError(f"Cannot add amounts in different currencies: {self.currency.code} + {other.currency.code}")
        return Amount(self.value + other.value, self.currency)
    
    def __sub__(self, other: Amount) -> Amount:
        if self.currency.code != other.currency.code:
            raise ValueError(f"Cannot subtract amounts in different currencies: {self.currency.code} - {other.currency.code}")
        return Amount(self.value - other.value, self.currency)
    ...
    def __lt__(self, other: Amount) -> bool:
        if self.currency.code != other.currency.code:
            raise ValueError(f"Cannot compare amounts in different currencies: {self.currency.code} < {other.currency.code}")
