...
from .accounts import Account, AccountScope
...
class JournalEntry:
    ...
    def _validate_zero_sum(self) -> None:
        ...
        for currency, total in currency_totals.items():
            if total != 0:
                raise ValueError(f"Entry {self.id} is not zero-sum for currency {currency}: {total}")
...
class Journal:
    ...
    def __init__(self, account_registry: Optional[Any] = None):
        ...
        self._balances: Dict[str, Dict[str, Decimal]] = {}  # account_id -> currency -> balance
    ...
    def balance(self, account_id: str, currency: str, at_timestamp: Optional[datetime] = None) -> Decimal:
        """
        ...
        for entry in self.entries:
            if entry.timestamp <= at_timestamp:
                for posting in entry.postings:
                    if (posting.account_id == account_id and 
                        posting.amount.currency.code == currency):
    ...
    def trial_balance(self, at_timestamp: Optional[datetime] = None) -> Dict[str, Dict[str, Decimal]]:
        """
    ...
    def cashflow(self, start_timestamp: datetime, end_timestamp: datetime, 
                 by_scope: Optional[AccountScope] = None) -> Dict[str, Decimal]:
        """
            ...
            for entry in self.entries:
                for posting in entry.postings:
                    if not account_registry.has_account(posting.account_id):
                        errors.append(f"Entry {entry.id}: Orphan account {posting.account_id}")
    ...
    def get_entries_by_time_range(self, start: datetime, end: datetime) -> List[JournalEntry]:
...
def generate_transaction_id(brick_id: str, timestamp: datetime, spec: Dict[str, Any], 
                          links: Dict[str, Any], sequence: int = 0) -> str:
    """
    ...
    content = f"{brick_id}:{timestamp_str}:{str(sorted(spec.items()))}:{links_str}:{sequence}"
