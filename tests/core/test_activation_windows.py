...
class TestActivationWindows:
    ...
    def test_multiple_overlapping_windows(self):
        ...
        for month in [3, 4, 5]:  # indices for months 4-6
            assert income_cash_in[month] > 0, f"Month {month+1}: income should be active"
            assert expense_cash_out[month] > 0, f"Month {month+1}: expense should be active"
    ...
    def test_window_end_event_generation(self):
        ...
        assert event_time == expected_time, f"Event time {event_time} != expected {expected_time}"
