...
class TestJournalInvariants:
    ...
    def test_zero_sum_entries(self):
        ...
        with pytest.raises(ValueError, match="not zero-sum"):
            invalid_entry = JournalEntry(
                ...
                postings=[
                    Posting("income", create_amount(-1000, "EUR"), {"type": "income"}),
                    Posting("cash", create_amount(500, "EUR"), {"type": "cash_in"})  # Not zero-sum
                ],
                metadata={"type": "income"}
            )
    ...
    def test_trial_balance(self):
        ...
        registry.register_account(Account("cash", "Cash", AccountScope.INTERNAL, AccountType.ASSET))
        registry.register_account(Account("income", "Income", AccountScope.BOUNDARY, AccountType.INCOME))
        registry.register_account(Account("expense", "Expense", AccountScope.BOUNDARY, AccountType.EXPENSE))
        ...
        total = sum(balances.get("EUR", Decimal("0")) for balances in trial_balance.values())
    ...
    def test_scenario_journal_integration(self):
        ...
        income = entity.new_FBrick('income', 'Salary', K.F_INCOME_FIXED, {'amount_monthly': 2000.0})
        expense = entity.new_FBrick('expense', 'Rent', K.F_EXPENSE_FIXED, {'amount_monthly': 800.0})
