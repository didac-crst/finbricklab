...
class BrickCompiler:
    ...
    def compile_tbrick(self, brick: TBrick, ctx: ScenarioContext, sequence: int = 0) -> List[JournalEntry]:
        """
        ...
        if "fx" in brick.spec:
            ...
            postings = [p for p in postings if p.account_id != to_account or p.amount.value > 0]
            postings.append(Posting(to_account, +dest_amount_obj, {"type": "fx_transfer_in"}))
            ...
            if residual != 0:
                ...
                postings.append(Posting(pnl_account, +residual_amount_obj, {"type": "fx_pnl"}))
        ...
        txn_id = generate_transaction_id(brick.id, timestamp, brick.spec, brick.links, sequence)
    ...
    def compile_fbrick(self, brick: FBrick, ctx: ScenarioContext, sequence: int = 0) -> List[JournalEntry]:
        """
        ...
        if is_income:
            ...
            boundary_posting = Posting(boundary_account, -amount_obj, {"type": "income"})
        else:
            boundary_account = f"Expenses:{brick.name.replace(' ', '_')}"
            boundary_posting = Posting(boundary_account, +amount_obj, {"type": "expense"})
        ...
        if not self.account_registry.has_account(boundary_account):
            ...
            self.account_registry.register_account(
                Account(boundary_account, boundary_account, AccountScope.BOUNDARY, account_type)
            )
        ...
        for account_id, allocation in internal_accounts.items():
            ...
            if is_income:
                internal_postings.append(Posting(account_id, +allocated_amount_obj, {"type": "income_allocation"}))
            else:
                internal_postings.append(Posting(account_id, -allocated_amount_obj, {"type": "expense_payment"}))
    ...
    def compile_abrick_valuation(self, brick: ABrick, ctx: ScenarioContext, 
                               old_value: Decimal, new_value: Decimal, 
                               sequence: int = 0) -> List[JournalEntry]:
        """
        ...
        if valuation_change > 0:
            ...
            postings = [
                Posting(asset_account, +change_amount_obj, {"type": "valuation_increase"}),
                Posting(pnl_account, -change_amount_obj, {"type": "unrealized_gain"})
            ]
        else:
            # Asset decreased in value
            postings = [
                Posting(asset_account, change_amount_obj, {"type": "valuation_decrease"}),
        ...
        txn_id = generate_transaction_id(f"{brick.id}_valuation", timestamp, brick.spec, brick.links, sequence)
    ...
    def _get_internal_accounts(self, brick: FBrick, ctx: ScenarioContext) -> Dict[str, Decimal]:
        ...
        if not brick.links:
            ...
            cash_accounts = [b for b in ctx.registry.values() if isinstance(b, ABrick) and b.kind == "a.cash"]
        ...
        if isinstance(to_accounts, str):
            ...
            if abs(total_allocation - Decimal("1.0")) > Decimal("0.001"):
                raise ValueError(f"Flow allocations must sum to 1.0, got {total_allocation}")
