...
```mermaid
graph LR
  subgraph "Shells (no balances)"
    FS[FlowShellBrick fs:*]
    TS[TransferShellBrick ts:*]
  end

  subgraph "Internal Nodes (A/L)"
    A[a:* Asset]
    L[l:* Liability]
  end
...
classDiagram
  ...
  class Shell {
    <<FlowShell or TransferShell>>
    +parent_id: str
  }
  Shell --> CDPair : creates
  Posting --> Node : hits
  CDPair o-- Posting
```

MacroGroup cancellation

```mermaid
flowchart TD
  S[Expand MacroGroup -> set S of A/L nodes]
  E[For each CDPair]
  C{Any posting hits b:boundary?}
  I{Both postings INTERNAL and node_id in S?}
  A{Asset posting node_id in S?}
  OUT[Include ASSET posting as cashflow]
  SKIP[Cancel entry]
...
Scenario aggregation (simplified)

```mermaid
sequenceDiagram
  participant SC as Scenario
  participant JR as Journal
  participant RES as ScenarioResults

  SC->>JR: collect CDPairs during simulate()
  SC->>RES: pass (outputs, journal, registry)
  RES->>RES: expand MacroGroup -> A/L set S
  RES->>JR: iterate entries by month
  JR-->>RES: two postings (DR/CR) + metadata
  RES->>RES: apply cancellation rules and sum ASSET postings
  RES-->>SC: monthly DataFrame (cash_in/out, assets, liabilities, equity)
```
