...
class Registry:
    ...
    def __init__(self, bricks: Dict[str, "FinBrickABC"], macrobricks: Dict[str, "MacroBrick"]):
        """
    ...
    def validate(self) -> "ValidationReport":
        ...
        for brick_id in self._bricks.keys():
            for prefix in reserved_prefixes:
                if brick_id.startswith(prefix):
                    report.id_conflicts.append(f"Brick ID '{brick_id}' uses reserved prefix '{prefix}'")
        
        for macrobrick_id in self._macrobricks.keys():
            for prefix in reserved_prefixes:
                if macrobrick_id.startswith(prefix):
                    report.id_conflicts.append(f"MacroBrick ID '{macrobrick_id}' uses reserved prefix '{prefix}'")
        ...
        if conflicts:
            report.id_conflicts.extend([f"ID conflict: '{id}'" for id in sorted(conflicts)])
            # Still raise for backward compatibility
            raise ConfigError(f"ID conflicts between bricks and MacroBricks: {sorted(conflicts)}")
        ...
        for macrobrick_id, macrobrick in self._macrobricks.items():
            try:
                ...
                # Still raise for backward compatibility
                raise ConfigError(f"Validation failed for MacroBrick '{macrobrick_id}': {e}") from e
