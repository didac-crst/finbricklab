...
from finbricklab.core.bricks import ABrick, LBrick, FBrick
...
class TestCashFlowRouting:
    ...
    def test_all_cash_flows_route_to_cash_account(self):
        ...
        assert np.sum(income_output["cash_in"]) > 0, "Income should generate cash inflows"
        
        # Cash account should receive external_in equal to income cash_in
        external_in = cash.spec.get("external_in", np.zeros(6))
        assert np.allclose(external_in, income_output["cash_in"], atol=1e-6), \
        ...
        assert np.sum(expense_output["cash_out"]) > 0, "Expense should generate cash outflows"
        
        # Cash account should receive external_out equal to expense cash_out
        external_out = cash.spec.get("external_out", np.zeros(6))
        assert np.allclose(external_out, expense_output["cash_out"], atol=1e-6), \
    ...
    def test_cash_balance_calculation(self):
        ...
        assert abs(cash_balance[0] - expected_first) < 1e-6, \
        ...
        assert cash_balance[-1] >= expected_final * 0.98, \
    ...
    def test_multiple_cash_flows_accumulate(self):
        ...
        cash = ABrick(
            ...
            name="Cash Account",
            kind=K.A_CASH,
            spec={"initial_balance": 0.0, "interest_pa": 0.0}  # No interest for clean test
        )
        ...
        assert np.allclose(external_in, total_income, atol=1e-6), \
        ...
        assert np.allclose(external_out, total_expenses, atol=1e-6), \
        ...
        for i in range(1, len(cash_balance)):
            ...
            assert abs(cash_balance[i] - expected_balance) < 1e-6, \
    ...
    def test_cash_account_interest_compounding(self):
        ...
        assert cash_balance[-1] > balance_without_interest, \
...
class TestCashRoutingIntegration:
    ...
    def test_property_purchase_with_mortgage_routing(self):
        ...
        assert np.sum(house_output["cash_out"]) > 0, "House should generate cash outflow"
        
        # Mortgage should generate cash outflow (payments)
        assert np.sum(mortgage_output["cash_out"]) > 0, "Mortgage should generate cash outflow"
        ...
        assert np.allclose(external_out, expected_external_out, atol=1e-6), \
            "Cash account should receive all cash outflows"
        
        # Verify cash balance decreases due to property purchase
        cash_balance = cash_output["asset_value"]
        assert cash_balance[0] < cash.spec["initial_balance"], \
